...
class Qwen2MoeSparseMoeBlock(nn.Module):
    def __init__(self, config):
        ...
        self.num_group = 1
        self.num_experts_per_group = self.num_experts // self.num_group
        ...
        self.expert_groups = nn.ModuleList(
            [ExpertGroup(config, self.num_experts_per_group) for _ in range(self.num_group)]
        )
        if self.num_group > 1:
            # Gate
            self.dialect_gate = nn.Linear(config.hidden_size, self.num_group, bias=False)
            torch.nn.init.zeros_(self.dialect_gate.weight)            
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if self.num_group > 1:
            # select expert group
            dialect_logits = self.dialect_gate(hidden_states).mean(dim=1)
            group_labels = torch.argmax(dialect_logits, dim=-1)

            # 根据分组标签创建索引列表
            groups = [[] for _ in range(self.num_group)]
            ...
            for gid, group in enumerate(groups):
                ...
                processed_group, groups_gate_logit = self.expert_groups[gid](group_tensor)
                processed_groups.append(processed_group)
                groups_gate_logits.append(groups_gate_logit)
            ...
            # # 合并回原始顺序
            final_hidden_states = torch.cat(processed_groups, dim=0)[sorted_indices]
            router_logits = torch.cat(groups_gate_logits, dim=0)[sorted_indices]
        else:
            final_hidden_states, router_logits = self.expert_groups[0](hidden_states)
        ...
        router_logits = router_logits.view(-1, self.num_experts_per_group)
        return final_hidden_states, router_logits
